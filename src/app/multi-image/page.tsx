...
export default function ImagePreview() {
  useEffect(() => {
    // Small delay to ensure DOM is fully ready and CSS is applied
    setTimeout(() => {
      initApp();
      ...
      <section className="hero" style={{ paddingBottom: 0 }}>
        <div className="wrap">
          ...
          <div className="tabs" style={{ marginTop: '24px' }}>
            <a href="/image" className="tab">Single Image</a>
            <a href="/multi-image" className="tab active">Multi Images</a>
          </div>

          {/* TOOL */}
          <div className="tool" id="tool">
            <div className="dropzone" id="uploadArea">
              ...
              <p>or click to browse — processing happens 100% on your device</p>
              <div className="formats">
                <span className="fmt">JPG</span><span className="fmt">PNG</span>
                <span className="fmt">WebP</span><span className="fmt">Bulk</span>
              </div>
              <input type="file" id="fileInput" accept="image/jpeg,image/png,image/webp" style={{ display: 'none' }} multiple />
            </div>

            <div className="compare" id="staticCompare">
              <div className="shot before">
                <span className="label">Before</span>
                <div className="photo">Original image</div>
                <span className="watermark">✦ Gemini</span>
              </div>
              <div className="shot after">
                <span className="label">After</span>
                <div className="photo">Watermark removed</div>
              </div>
            </div>

            <div id="fileQueuePanel" className="compare" style={{ display: 'none', flexDirection: 'column', gap: '12px', marginTop: '28px', background: 'var(--card)', border: '1px solid var(--border)', borderRadius: '16px', padding: '16px' }}>
              <div style={{ fontWeight: 700, paddingBottom: '12px', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span>Multi Processing Queue</span>
                <button id="downloadAllBtn" className="btn btn-primary" style={{ padding: '6px 14px', fontSize: '13px', display: 'none' }}>Download All ZIP</button>
              </div>
              <div id="fileQueueList" style={{ display: 'flex', flexDirection: 'column', gap: '8px', maxHeight: '350px', overflowY: 'auto' }}>
                {/* File items injected here by JS */}
              </div>
            </div>

            <div id="singlePreview" style={{ display: 'none', marginTop: '28px' }}>
              <div className="relative w-full flex-1 flex items-center justify-center select-none group min-h-[400px] rounded-[16px] border border-[#e6e8ef] overflow-hidden" style={{ backgroundImage: "url('data:image/svg+xml;base64,PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciIHdpZHRoPSIyMCIgaGVpZ2h0PSIyMCIgZmlsbD0iI2Y5ZmRmZCI+PHJlY3Qgd2lkdGg9IjEwIiBoZWlnaHQ9IjEwIiBmaWxsPSIjZjJmMmYyIi8+PHJlY3QgeD0iMTAiIHk9IjEwIiB3aWR0aD0iMTAiIGhlaWdodD0iMTAiIGZpbGw9IiNmMmYyZjIiLz48L3N2Zz4=')" }}>
                <div id="comparisonContainer" className="relative mx-auto w-fit max-w-full">
                  <img id="originalImage" className="block w-auto h-auto max-w-full max-h-[65vh] object-contain object-right-bottom" draggable="false" />
                  <div id="processedOverlay" className="absolute top-0 left-0 h-full w-[50%] overflow-hidden border-r-[1.5px] border-white shadow-[1px_0_4px_rgba(0,0,0,0.15)] box-content z-10" style={{ display: 'none' }}>
                    <img id="processedImage" className="block h-full max-w-none object-contain object-right-bottom" draggable="false" />
                  </div>
                  <div id="sliderHandle" className="absolute top-0 bottom-0 w-10 -ml-5 z-20 cursor-col-resize flex items-center justify-center group-hover:opacity-100 transition-opacity" style={{ left: '50%', display: 'none' }}>
                    <div className="absolute w-7 h-7 bg-white rounded-full shadow-[0_1px_3px_rgba(0,0,0,0.2)] flex items-center justify-center border border-[#e6e8ef]">
                      <svg className="w-3.5 h-3.5 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M8 9l-3 3 3 3m8-6l3 3-3 3"></path></svg>
                    </div>
                  </div>
                  <div className="absolute top-3 left-3 backdrop-blur text-xs font-medium px-2 py-0.5 rounded shadow-sm border pointer-events-none z-30" style={{ background: 'rgba(var(--header-bg-rgb), 0.9)', color: 'var(--text)', borderColor: 'var(--border)' }}>Processed</div>
                  <div className="absolute top-3 right-3 backdrop-blur text-xs font-medium px-2 py-0.5 rounded shadow-sm border pointer-events-none z-30" style={{ background: 'rgba(var(--header-bg-rgb), 0.9)', color: 'var(--text)', borderColor: 'var(--border)' }}>Original</div>
                </div>
              </div>
              <div id="originalInfo" style={{ display: 'none' }}></div>
              <div id="processedInfo" style={{ display: 'none' }}></div>

              <div style={{ display: 'flex', gap: '12px', justifyContent: 'center', margin: '24px auto' }}>
                <button id="downloadBtn" className="btn btn-primary" style={{ display: 'none' }}>
              ...
              <div id="statusMessage" style={{ textAlign: 'center', marginTop: '12px', fontSize: '14px', color: 'var(--muted)' }}></div>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="seo-content" style={{ marginTop: '60px', padding: '60px 0', background: 'var(--bg-soft)' }}>
        <div className="wrap">
          <div className="sec-head" style={{ textAlign: 'center', marginBottom: '48px', maxWidth: '720px', margin: '0 auto 48px' }}>
            ...
            <h2 style={{ fontSize: 'clamp(28px, 4vw, 42px)', fontWeight: 800, marginTop: '12px', letterSpacing: '-1px' }}>The Best Free Gemini Image Watermark Remover</h2>
            <p style={{ color: 'var(--muted)', marginTop: '14px', fontSize: '17px' }}>
              Remove watermarks from your AI-generated images without compromising quality or privacy. Our tool runs directly in your browser, meaning your photos are never uploaded to any server.
            </p>
          </div>
          ...
          <div className="grid-features" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 300px), 1fr))', gap: '24px' }}>
            <div className="feature-card" style={{ background: 'var(--card)', padding: '32px', borderRadius: '20px', boxShadow: '0 4px 24px rgba(0,0,0,0.04)', border: '1px solid var(--border)' }}>
              ...
              <h3 style={{ fontSize: '18px', fontWeight: 700, marginBottom: '12px' }}>Pixel-Accurate Restoration</h3>
              <p style={{ color: 'var(--muted)', fontSize: '14px', lineHeight: 1.6 }}>We don't use AI repainting. Our algorithm mathematically reverses the watermark's alpha blend to recover the exact original pixels underneath.</p>
            </div>
            ...
            <div className="feature-card" style={{ background: 'var(--card)', padding: '32px', borderRadius: '20px', boxShadow: '0 4px 24px rgba(0,0,0,0.04)', border: '1px solid var(--border)' }}>
              ...
              <p style={{ color: 'var(--muted)', fontSize: '14px', lineHeight: 1.6 }}>Since everything happens locally in your browser, your personal images are completely safe and are never sent over the internet.</p>
            </div>

            <div className="feature-card" style={{ background: 'var(--card)', padding: '32px', borderRadius: '20px', boxShadow: '0 4px 24px rgba(0,0,0,0.04)', border: '1px solid var(--border)' }}>
              <div className="f-icon" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: '48px', height: '48px', borderRadius: '12px', background: 'rgba(245, 158, 11, 0.1)', color: 'var(--warn)', marginBottom: '20px' }}>
                <Zap size={24} />
              </div>
              <h3 style={{ fontSize: '18px', fontWeight: 700, marginBottom: '12px' }}>Bulk Processing</h3>
              <p style={{ color: 'var(--muted)', fontSize: '14px', lineHeight: 1.6 }}>Drop dozens of JPG, PNG, or WebP images at once. We'll clean them all instantly using parallel processing threads and give you a ZIP file.</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="how-section" style={{ padding: '80px 0', background: 'var(--bg)' }}>
        <div className="wrap">
          <div className="sec-head" style={{ textAlign: 'center', marginBottom: '48px', maxWidth: '720px', margin: '0 auto 48px' }}>
            ...
            <h2 style={{ fontSize: 'clamp(28px, 4vw, 38px)', fontWeight: 800, letterSpacing: '-1px', marginTop: '12px' }}>How to Remove Image Watermarks Online</h2>
          </div>
          <div className="steps-container" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 280px), 1fr))', gap: '24px' }}>
            <div className="step-box" style={{ padding: '28px', border: '1px solid var(--border)', background: 'var(--card)', borderRadius: '16px', boxShadow: '0 1px 3px rgba(16,24,40,.04)' }}>
              ...
              <h3 style={{ fontSize: '18px', fontWeight: 700, marginBottom: '8px' }}>Upload Photos</h3>
              <p style={{ color: 'var(--muted)', fontSize: '14px' }}>Drag and drop your JPG, PNG, or WebP file into the workspace above. Bulk uploads are supported.</p>
            </div>
            <div className="step-box" style={{ padding: '28px', border: '1px solid var(--border)', background: 'var(--card)', borderRadius: '16px', boxShadow: '0 1px 3px rgba(16,24,40,.04)' }}>
              ...
              <p style={{ color: 'var(--muted)', fontSize: '14px' }}>In-browser processing instantly detects and strips the Gemini logo, star overlay, or Nano Banana watermark.</p>
            </div>
            <div className="step-box" style={{ padding: '28px', border: '1px solid var(--border)', background: 'var(--card)', borderRadius: '16px', boxShadow: '0 1px 3px rgba(16,24,40,.04)' }}>
              ...
              <p style={{ color: 'var(--muted)', fontSize: '14px' }}>Once finished, instantly download your crisp, watermark-free images as PNGs or grab them all in a ZIP archive.</p>
            </div>
          </div>
        </div>
      </section>
